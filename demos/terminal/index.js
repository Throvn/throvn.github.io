var _this = this;
var $cmd = document.querySelector(".window-content").lastElementChild
  .lastElementChild;
  ...
  commands: {
    ...
    clear: {
      ...
      func: function (path, args) {
        ...
        $cmd = document.querySelector(".window-content").lastElementChild
          .lastElementChild;
        return "";
      },
    },
      ...
      func: function (path, args) {
        ...
        for (var i = 0; i < pth.length; i++) {
          ...
          if (typeof initFS !== "object") {
            return (
              '<span style="margin-right: 8px;" class="text-error">\u2718</span> ls: ' +
              args[1] +
              ": no such directory"
            );
          }
    ...
    cd: {
      ...
      func: function (currPath, args) {
        ...
        for (var i = 0; i < pth.length; i++) {
          ...
          if (!initFS) {
            return (
              '<span style="margin-right: 8px;" class="text-error">\u2718</span> cd: ' +
              args[1] +
              ": does not exist"
            );
          } else if (
        ...
        path =
          currentTree.length > 0 ? currentTree[currentTree.length - 1] : "root";
      },
    },
    path: {
    ...
    mkdir: {
      ...
      func: function (path, args) {
        ...
        return (
          '<span style="margin-right: 8px;" class="text-success">\u2714</span> Success: Directory ' +
          args[1].replace("/", "-") +
          " created."
        );
      },
      ...
      func: function (path, args) {
        ...
        if (typeof file === "object") {
          if (file.meta.type === "download") {
            ...
            downloadEL.click();
            return (
              '<span style="margin-right: 8px;" class="text-success">\u2714</span> Success: The download should have started now. <br> Alternatively click <a href="' +
              file["link"] +
              '" download="' +
              file["name"] +
              '">here.'
            ...
            return (
              '<span style="margin-right: 8px;" class="text-error">\u2718</span> Error: /' +
              args[1] +
              " is not a file."
            );
          }
          ...
          return (
            '<span style="margin-right: 8px;" class="text-error">\u2718</span> Error: The file /' +
            args[1] +
            " does not exist."
          );
        } else if (typeof file === "string") {
    ...
    notFound: {
      ...
      func: function (path, args) {
        return (
          '<span style="margin-right: 8px;" class="text-error">\u2718</span> Error: command not found: ' +
          args[0]
        );
      },
    },
    ...
    root: {
      ...
      projects: {
        ...
        old_website: {
          ...
          link: "/index_normal.html",
        },
      },
    },
  },
};
var currFolder = structure.filesystem.root,
  lastFolder = structure.filesystem.root;
function checkCommands(rawCmd) {
  ...
  var result = command
    ? command.func(path, cmd)
    : structure.commands["notFound"].func(path, cmd);
...
document.body.onkeyup = function (evt) {
  if (evt.key === "Tab") {
    ...
    if (tmpInp_1.length === 1) {
      ...
      var autoComplete = commandNames.filter(function (name) {
        return name.lastIndexOf(tmpInp_1[0], 0) === 0;
      });
      ...
      var autoComplete = Object.keys(
        getFolder(currentTree.concat(previousTree))
      ) // get all contents of the current directory
        .filter(function (name) {
          return name.lastIndexOf(searchWord_1, 0) === 0;
        }); // search for filenames/directories which start with the searchword
      if (autoComplete.length === 1) {
